...
function get_extended_solution(num_pts, X_domain, X_boundary, L, sol, set_sigma)
    ...
    xxv = Matrix(reshape(xx,(num_pts^2,1)))
    yyv = Matrix(reshape(yy,(num_pts^2,1)))
...
#     X_test=hcat([[x[i];y[j]] for i = 1 : num_pts for j = 1 : num_pts])
    ...
    u_extended = Matrix(reshape(extend_solution(X_test, X_domain, X_boundary, L, temp_vec, set_sigma),(num_pts,num_pts)))
